document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    message.innerHTML = '<div class="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800">Dang xac thuc tai khoan...</div>';

    try {
      const payload = await window.AppApi.login(identity, password);
      window.AppState.setSession(payload.token, payload.teacher);
      message.innerHTML = '<div class="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">Dang nhap thanh cong. Dang chuyen sang dashboard.</div>';
      window.setTimeout(() => {
